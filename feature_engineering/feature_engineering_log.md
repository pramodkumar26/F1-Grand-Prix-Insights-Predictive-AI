...
## 2026 season added, and two real bugs caught pulling it in

Extended the collection pipeline (`pull_schedule.py`, `qualifying.py`, `race_control.py`, `retry.py`) to include 2026, re-ran the established load order against the combined 2018-2026 data. Two of the loaders had never been run against anything but a single new year's worth of data before, and both had a latent assumption that broke once the input spanned old and new years together.

**`load_circuits_races.py` had no duplicate check on `dim_race`.** `dim_circuit` used `INSERT OR IGNORE`, `dim_race` used a plain `INSERT`, safe only because every previous run's `all_schedule.csv` happened to contain exactly one new year. Running it against the now-combined 2018-2026 schedule would have duplicated every existing race. Caught before running, not after, by checking the dim_race schema for a uniqueness constraint (there isn't one) and reasoning about what a plain INSERT against the full file would do. Fixed by checking `(year, round)` against what's already loaded before inserting, same fix applied to all five fact table loaders (`load_laps.py`, `load_results.py`, `load_weather.py`, `load_qualifying.py`, `load_race_control.py`), all of which had the identical latent assumption. Verified after running: `dim_race` grew by exactly 23 (196 total, was 173), every fact table's skip count matched its pre-existing row count exactly, confirming nothing old got touched.

**`load_weather.py` inserted `race_id` as a raw byte blob, not an integer.** Every other loader explicitly casts the race lookup to `int()` before using it as a query parameter; this one didn't, and passed a numpy int64 straight through. sqlite3's parameter binding silently accepted it via the buffer protocol instead of raising an error, storing the 8-byte raw representation as a BLOB (`typeof()` confirmed it directly, `\xae\x00\x00\x00\x00\x00\x00\x00` decodes to 174 in little-endian). Old rows, loaded before this run, were unaffected and correctly typed as `integer`, this only hit the newly inserted 2026 rows, all 1,750 of them. Caught because a downstream script (`fix_weather_session_time.py`) does a row-count reconciliation before trusting its own positional alignment, and it aborted on a mismatch instead of silently writing corrupted data on top of corrupted data. Deleted the 1,750 blob rows, added the missing `int()` cast, reloaded, confirmed clean.

**`fix_lap_time_columns.py` reinterpreted 185,326 already-correct lap times as nanoseconds.** The bigger of the two, because it corrupted historical data that had been correct for the entire project up to this point, not just newly inserted rows. The script's per-column numeric check (`if pd.api.types.is_numeric_dtype(series)`) was written assuming a column is either entirely already-fixed floats or entirely raw timedelta strings. Once `fact_laps` contained old float rows sitting alongside new 2026 text rows in the same SQL column, `pd.read_sql` returned the whole column as `object` dtype (SQLite has per-row, not per-column, typing), so the numeric check failed for the *entire* column and every row, including the already-correct old ones, got run through `pd.to_timedelta`. Passing a bare number like `91.2` (already seconds) into `to_timedelta` with no unit specified gets interpreted as nanoseconds, so `91.2` seconds became `91.2` nanoseconds, `9.1e-8` seconds. Caught immediately, the script's own printed `describe()` output showed a median lap time of `9.1e-8` seconds, physically impossible, checked before trusting it rather than moving on.

Recovered from a clean git-committed snapshot of `f1.db` (restoring `lap_time`/`pit_in_time`/`pit_out_time` for every `lap_id` that existed before this run's insert, keyed by `lap_id`, using the project's own established staged-UPDATE pattern, the same one `fix_grid_position.py` and this script itself already use). First recovery attempt used a cross-database `ATTACH` with a correlated subquery and silently wrote the same single value to every row instead of a per-row match, caught by checking for value diversity after the "fix" (`COUNT(DISTINCT lap_time)` came back as 1), not assumed to have worked just because it ran without error. Redone with the proven single-database staged-table pattern instead, verified correctly this time (52,712 distinct values restored, mean 91.47s matching the clean snapshot exactly).

Fixed the root cause, not just the immediate damage: `convert_to_seconds()` now checks numeric-vs-text per value (`pd.to_numeric(series, errors='coerce')`, only falling back to timedelta parsing for whatever remains unparsed) instead of once for the whole column, so this can't recur when 2027's data gets added on top of a column that's already a mix of fixed and unfixed rows.

**Lesson, in the same spirit as the row-count-against-source lesson already in this file:** every loader and fix script in this project had been run exactly once per year of new data until this pass. Re-running one against a column or table that now mixes old, already-processed rows with new, unprocessed ones is a genuinely different situation than the one each script was originally written and tested for, and "it worked before" is not evidence it's still safe once the input shape changes. Worth checking this same class of assumption in every script before pulling 2027. 