...
def load_data(conn):
    return pd.read_sql('SELECT * FROM tyre_degradation_features', conn)
...
def main():
    ...
    baseline_mean = mean_absolute_error(y_test, [y_train.mean()] * len(y_test))
    print(f'trivial mean-predictor MAE: {baseline_mean:.4f}')
